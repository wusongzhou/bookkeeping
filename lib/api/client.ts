...
export const api = {
  ...
  auth: {
    login: async (username: string, password: string) => {
      const response = await request<ApiResponse<{ token: string; user: { id: string; username: string } }>>(
        '/api/auth/login',
        {
          method: 'POST',
  ...
  // 物品相关
  items: {
    getAll: async (filter?: { archived?: number }) => {
      ...
      return request<ApiResponse<Item[]>>(url);
    },
  ...
  tags: {
    ...
    getItemTags: async (itemId: number) => {
      return request<ApiResponse<import('../types').Tag[]>>(`/api/items/${itemId}/tags`);
    },
